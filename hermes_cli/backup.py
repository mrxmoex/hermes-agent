...
_EXCLUDED_DIRS = {
    ...
    "browser-profile",
    # Bot Desktop runtime: cookie jar (also hit by browser-profile), lease.json (a restored
    # holder=human fences computer_use with no screen), dock-cdp-port (a stale port fail-opens
    # leftover CDP classification), Xauthority, sockets, pids. Export / --clone-all /
    # profile-import already refuse this tree; full backup/import is the last archive door.
    "bot-desktop",
...
def _is_bot_desktop_rel(rel: str) -> bool:
    """True when *rel* sits under a ``bot-desktop/`` tree (root or named profile).

    Older full-home zips still carry this tree; skip them on import the same
    way ``import_profile`` drops a crafted profile archive.
    """
    return bool(rel) and "bot-desktop" in Path(rel).parts
...
def _import_members(
    ...
    for member in members:
        ...
        if external:
            ...
            if rel and (Path(rel).name in _IMPORT_SKIP_NAMES or _is_bot_desktop_rel(rel)):
