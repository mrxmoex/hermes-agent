...
def _screen_status(args) -> int:
    ...
    if bool(getattr(args, "json", False)):
        print(json.dumps({**st.as_dict(), "lease": lease.get().public_view()}, indent=2, sort_keys=True))
    ...
    if st.running:
        holder = lease.get()
        view = holder.public_view()
        who = f"human ({view['viewer_hash']})" if holder.holder == "human" else "agent"
